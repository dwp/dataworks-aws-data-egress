resource "aws_s3_object" "data_egress_sft_agent_config" {
...
resource "aws_s3_object" "data_egress_sft_agent_application_config" {
  bucket     = data.terraform_remote_state.common.outputs.config_bucket.id
...
data "template_file" "data_egress_sft_agent_application_config_tpl" {
  ...
  vars = {
    oph_destination_url = local.use_stub_nifi[local.environment] ? local.stub_nifi_alb_fqdn : data.terraform_remote_state.aws_sdx.outputs.sdx_f5_endpoint_1_name[0]
    aws_destination_url = local.use_stub_nifi[local.environment] ? local.stub_nifi_alb_fqdn : local.data_egress[local.environment].aws_hub_url
  }
}
