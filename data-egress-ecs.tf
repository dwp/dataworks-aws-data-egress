resource "aws_ecs_task_definition" "data-egress" {
  ...
  cpu                      = "2048"
  memory                   = "4096"
  task_role_arn            = aws_iam_role.data_egress_server.arn
...
data "template_file" "data_egress_definition" {
  ...
  vars = {
    ...
    cpu                = var.fargate_cpu
    image_url          = format("%s:%s", data.terraform_remote_state.management.outputs.dataworks_data_egress_url, var.image_versions.data-egress)
